...
public class OrderCardDeliveryTest {
    ...
    void shouldSubmitValidData3() {
        ...
        $("[data-test-id = phone] .input__control").setValue("+79112345678");
    ...
    void shouldSubmitNotValidCity() {
        String date = setLocalDate(3);
        $("[data-test-id=city] .input__control").setValue("Купчино");
        $("[data-test-id = date] .input__control").doubleClick().sendKeys(date);
        $("[data-test-id = name] .input__control").setValue("Иван Петров");
        $("[data-test-id = phone] .input__control").setValue("+79112345678");
        $("[data-test-id = agreement]").click();
...
// все остальные тесты для ФИО, телефона аналогично
}